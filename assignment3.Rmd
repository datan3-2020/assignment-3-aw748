...
author: "Amy Wonnacott [add your candidate number here - mandatory]"
date: "12.02.20"
...
files <- dir(
             ...
             "C:/Users/User/Documents/GitHub/dataan3/UKDA-6614-tab/tab",
...
My findings show that females are more likely to have higher political interest than males, a statement which is true for all 7 waves. The ifgures do not vary too much between the waves, which suggests that political interest does not vary to a high degree per year. The highest mean of political interest for men and women occurred in Wave D: 2.98 for females and 2.55 for males. Wave D was in 2012-2013, which may have caused high political interest due to the announcing of both a Scottish and EU referendum, and the London mayoral elections. 
The lowest mean for both men and women occurred in Wave G, with a mean of 2.73 for females and 2.42 for males. Wave G was in 2015-2016, which had a large number of siginifican political elections and events such as the 2015 General Election and the 2016 EU referendum. However this may cause lower political interest due to dissatisfied voters and lack of interest or trust in politics. 
...
```{r}
# creating a new dataset witgout 
Delta<- Long %>%
  na.omit(Long)%>%
  select(pidp, wave, vote6)

pidp.unique<- Delta%>%
  distinct(pidp)

pidp1<- Delta%>%
  filter(pidp=="68004087")

delta<- function(pidp.unique){
  sum(Delta$vote6[Delta$wave=="b"|Delta$pidp==pidp.unique]-Delta$vote6[Delta$wave=="a"|Delta$pidp==pidp.unique])+
    (Delta$vote6[Delta$wave=="c"|Delta$pidp==pidp.unique]-Delta$vote6[Delta$wave=="b"|Delta$pidp==pidp.unique])+
    (Delta$vote6[Delta$wave=="d"|Delta$pidp==pidp.unique]-Delta$vote6[Delta$wave=="c"|Delta$pidp==pidp.unique])+
    (Delta$vote6[Delta$wave=="e"|Delta$pidp==pidp.unique]-Delta$vote6[Delta$wave=="d"|Delta$pidp==pidp.unique])+
    (Delta$vote6[Delta$wave=="f"|Delta$pidp==pidp.unique]-Delta$vote6[Delta$wave=="e"|Delta$pidp==pidp.unique])+
    (Delta$vote6[Delta$wave=="g"|Delta$pidp==pidp.unique]-Delta$vote6[Delta$wave=="f"|Delta$pidp==pidp.unique])}
```
Calculating mean Delta for men and women using the meanVote6 dataset created earlier.

```{r}
meanVote6<- meanVote6%>%
  mutate(mean= sum(b-a)+(c-b)+(d-c)+(e-d)+(f-e)+(g-f))
